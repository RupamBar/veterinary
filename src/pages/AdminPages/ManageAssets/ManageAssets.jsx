...
function ManageAssets() {
  ...
  // ***for updating user
  const startEditAsset = async (data) => {
    try {
      // console.log(data.id, "dATA");
      const res = await axios.get(
        `${config.backend_URL}/getAssetById/${data.id}`
      );
  ...
  const createAsset = async () => {
    try {
      ...
      const res = await axios.get(
        `${config.backend_URL}/getAssetByName/${newUser.name}`
      );
      if (res.data.data.length === 0) {
        const resp = await axios.post(
          `${config.backend_URL}/addAsset`,
          newUser
        );
        toast.success("Asset is added successfully", {
          theme: "colored",
        });
        handleClose();
        getAllAssets();
  ...
  const startDeleteAsset = async (data) => {
    const ans = window.prompt(
      `Do you want to delete the asset ${data.name} ? If yes then put 'YES' here and click OK`
    );
    // console.log(ans, "ans");
    ...
    try {
      if (ans === "YES") {
        const res = await axios.delete(
          `${config.backend_URL}/deleteAsset/${data.id}`
        );
    ...
    <div>
      ...
      <div className="adminPageBg">
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <div className="adminPageHeading">Manage Assets</div>
        </div>
      ...
      >
        <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
          {flagToCallUpdateAPI ? "Update Asset" : "Add Asset"}
        </DialogTitle>
        <IconButton
          aria-label="close"
          onClick={handleClose}
          ...
          <Button
            ...
            onClick={(e) => {
              if (
                ...
                flagToCallUpdateAPI ? updateAsset() : createAsset();
              }
            }}
            style={{
              backgroundColor: "DodgerBlue",
