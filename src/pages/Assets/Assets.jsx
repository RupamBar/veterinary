...
function Assets() {
  ...
  const getFilteredFoodsList = async (txt) => {
    try {
      const res = await axios.get(
        `${config.backend_URL}/getAllAssetsByFilter/${txt}`
      );
      setAllAssetList(res.data.data || []);
    } catch (err) {
      console.log("Error == ", err.message);
  ...
  const searchAssetByName = async(val) => {
    try
    {
        const res = await axios.get(`${config.backend_URL}/searchAssetByName/${val}`);
        setAllAssetList(res.data.data || []);
    }
    catch(err)
    {
    ...
    <div>
      ...
      <div className="foodListContainer">
        <div className="headingNSearch">
            <h1 className="foodContainerHeading">Asset Collections</h1>
            <div>
                <TextField
                id="standard-basic"
                ...
                onChange={(e) => {
                    if(e.target.value)
                    {
                        searchAssetByName(e.target.value);
                    }
                    else
                    {
                        getAllAssets();
