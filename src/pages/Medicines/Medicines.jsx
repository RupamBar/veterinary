...
function Medicines() {
  ...
  const getAllFoods = async () => {
    try {
      ...
      setAllMedicineList(res.data.data || []);
    } catch (err) {
      console.log("Error == ", err.message);
    }
  };
  ...
  const getFilteredFoodsList = async (txt) => {
    try {
      const res = await axios.get(
        `${config.backend_URL}/getAllMedicinesByFilter/${txt}`
      );
      setAllMedicineList(res.data.data || []);
    } catch (err) {
      console.log("Error == ", err.message);
  ...
  const searchMedicineByName = async(val) => {
    try
    {
        const res = await axios.get(`${config.backend_URL}/searchMedicineByName/${val}`);
        setAllMedicineList(res.data.data || []);
    }
    catch(err)
    {
  ...
  return (
    <div>
      ...
      <div className="foodListContainer">
        ...
        <span style={{ cursor: "pointer" }}>&nbsp;&gt;&nbsp;Medicines</span>
      </div>
      <div className="foodListContainer">
        <div className="headingNSearch">
            <h1 className="foodContainerHeading">Medicines</h1>
            <div>
                <TextField
                id="standard-basic"
                ...
                onChange={(e) => {
                    if(e.target.value)
                    {
                        searchMedicineByName(e.target.value);
                    }
                    else
                    {
                        getAllFoods();
              ...
              return (
                <div key={i} className="card">
                  ...
                  <div className="productPrice">
                    ...
                    <div>
                      <del className="delText">Rs.{item?.price + 107}</del>Rs.
                      {item?.price}
                    </div>
                  </div>
