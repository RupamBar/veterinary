...
function AnimalFoods() {
  ...
  const getFilteredFoodsList = async (txt) => {
    try {
      const res = await axios.get(
        `${config.backend_URL}/getAllFoodsByFilter/${txt}`
      );
      setAllFoodList(res.data.data || []);
    } catch (err) {
      console.log("Error == ", err.message);
  ...
  const searchFoodByName = async(val) => {
    try
    {
        const res = await axios.get(`${config.backend_URL}/searchFoodByName/${val}`);
        setAllFoodList(res.data.data || []);
    }
    catch(err)
    {
  ...
  return (
    <div>
      ...
      <div className="foodListContainer">
        ...
        <span style={{ cursor: "pointer" }}>&nbsp;&gt;&nbsp;Animal Foods</span>
      </div>
      <div className="foodListContainer">
        <div className="headingNSearch">
            <h1 className="foodContainerHeading">Animal Foods</h1>
            <div>
                <TextField
                id="standard-basic"
                ...
                onChange={(e) => {
                    if(e.target.value)
                    {
                        searchFoodByName(e.target.value);
                    }
                    else
                    {
                        getAllFoods();
