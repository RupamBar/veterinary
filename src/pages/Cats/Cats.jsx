...
function Cats() {
  ...
  const getAllCats = async () => {
    try {
      const res = await axios.get(
        `${config.backend_URL}/getAllAnimalsByType/Cat`
      );
  ...
  const getFilteredAnimalList = async (txt) => {
    try {
      const res = await axios.get(
        `${config.backend_URL}/getAllAnimalsByFilter/Cat/${txt}`
      );
      setAllAnimalList(res.data.data || []);
    } catch (err) {
      console.log("Error == ", err.message);
  ...
  const searchAnimalByName = async (val) => {
    try {
      const res = await axios.get(
        `${config.backend_URL}/searchAnimalByName/Cat/${val}`
      );
      setAllAnimalList(res.data.data || []);
    } catch (err) {
      console.log("Error ==", err.message);
  ...
  return (
    <div>
      ...
      <div className="foodListContainer">
        ...
        <span style={{ cursor: "pointer" }}>&nbsp;&gt;&nbsp;Cats</span>
      </div>
      <div className="foodListContainer">
        <div className="headingNSearch">
          <h1 className="foodContainerHeading">Cat Collections</h1>
          <div>
            <TextField
              id="standard-basic"
              ...
              return (
                <div key={i} className="card">
                  ...
                  <div className="productPrice">
                    ...
                    <div>
                      <del className="delText">Rs.{item?.price + 2024}</del>Rs.
                      {item?.price}
                    </div>
                  </div>
